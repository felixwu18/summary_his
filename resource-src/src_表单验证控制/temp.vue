    ...
    <div style="width: 50%; margin: 0 auto">
      ...
      >
        ...
        <el-form-item label="活动区域" prop="region">
          <el-select v-model="ruleForm.region" :disabled="isDisable('ruleForm.region', ruleForm.name)" placeholder="请选择活动区域">
            <el-option label="区域一" value="shanghai"></el-option>
            <el-option label="区域二" value="beijing"></el-option>
          </el-select>
        </el-form-item>
        <!-- 测试静默阻止不合规内容 -->
        <!-- @blur="ruleForm.input=/^[0-9]+.?[0-9]*$/.test(ruleForm.input) ? ruleForm.input:'';" -->
        <!-- ruleForm.notNumber=ruleForm.notNumber.replace(/^[0-9]+.?[0-9]*$/,''); -->
        <el-form-item label="只能数字" prop="input">
          <el-input
            v-model="ruleForm.input"
            :disabled="isDisable('ruleForm.input', ruleForm.region)" 
...
export default {
  ...
  methods: {
    ...
    isDisable(controlObj, val) {
      switch (controlObj) {
        case undefined:
        case 'ruleForm.region':
          return ruleForm.name ? false : true;
        case 'ruleForm.input':
                      console.log(val === "1");
